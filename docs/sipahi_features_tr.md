...
**Versiyon:** v1.0 (U-20 sonrası) · **Mimari:** RISC-V RV64IMAC · **Dil:** Rust no_std
**Toplam:** ~9,132 Rust + ~321 ASM satır · 200 Kani harness · 7/7 TLA+ verified (35,770 distinct states)
...
IEEE 754 floating-point aritmetiği non-deterministic olabilir — farklı donanımlarda aynı hesaplama farklı sonuç üretebilir (rounding mode, denormalized number handling). Safety-critical'da bu kabul edilemez. Sipahi tüm hesaplamaları Q32.32 fixed-point (`i64`) ile yapar: ±2³¹ aralık, ~2.3×10⁻¹⁰ hassasiyet. WASM modüllerinde float opcode tespit edilirse modül reddedilir — `is_float_opcode()` byte-level heuristic ile 0x2a-0xbf aralığını + 0xFC prefix saturating truncation sub-opcode'larını tarar (tam WASM grammar parser değil, detay için bkz. §10.3).

### 1.4 Neden Microkernel?

Monolitik kernel yerine microkernel çünkü saldırı yüzeyi küçük. Kernel sadece scheduler, IPC, capability, policy ve trap handler içerir. WASM sandbox, blackbox, secure boot kernel dışında çalışır. Bir bileşen çökerse kernel ayakta kalır. DO-178C DAL-A **design principles** için küçük, doğrulanabilir kernel şart (sertifikasyon iddiası DEĞİL — sınırlamalar için bkz. ARCHITECTURE.md "Formal Verification Scope & Limitations") — 200 Kani harness (~100 symbolic + ~100 concrete/compile-time) ve 7/7 TLA+ spec ile kritik invariant'lar (scheduler seçim doğruluğu, policy escalation, IPC bütünlüğü, bellek güvenliği) bounded model checking ile doğrulanmış.
...
Sipahi sabit öncelikli (fixed-priority) preemptive scheduler kullanır. Round-robin veya EDF (Earliest Deadline First) yerine sabit öncelik çünkü WCET analizi daha kolay ve DO-178C **design principles** için tercih edilir. Priority 0 = en yüksek (DAL-A), priority 15 = en düşük (DAL-D).
...
`is_valid_user_ptr(caller_task_id, ptr, size)` task'a özeldir ve default-deny çalışır. Null pointer'ı reddeder, `checked_add` ile overflow'u yakalar, sonra aralığı yalnızca çağıran task'ın kendi stack NAPOT aralığına (`scheduler::task_stack_range(caller_task_id)`) tamamen sığıyorsa kabul eder. Dead/isolated/yok task'ların geçerli user pointer aralığı yoktur.

Alignment kontrolü IPC syscall handler'ları tarafından mesaj kopyalamadan önce yapılır. Başka bir task'ın stack'ine işaret eden pointer, o stack teknik olarak geçerli olsa bile reddedilir.

**Neden geniş RAM_END kontrolü değil task-specific kontrol?** Global RAM aralığı başka task'ın stack'ini yanlışlıkla geçerli sayabilir. Syscall sınırında kanıtlanması gereken şey "bu pointer RAM'de mi?" değil, "bu pointer çağıran task'a mı ait?" sorusudur. Böylece pointer kontrolü deterministik kalır ve PMP trap'ine düşmeden önce task izolasyonu korunur.
...
`BB_TICK` u32 her scheduler tick'te `wrapping_add(1)` ile artar. Wrap tespiti: `next < t` → `BB_BOOT_EPOCH` u16 artırılır. `get_tick()` → `(epoch << 32) | tick` = u48 etkili alan. 10ms/tick varsayımıyla u32 tick ~497 günde wrap eder, u16 epoch ile birleşince ~89,000 yıl monoton kalır — token expiry için kafi fazla.
...
### 10.3 Float Opcode Reddi (byte-level heuristic — tam WASM decoder DEĞİL)

`validate_module()` modül yüklenirken byte stream üzerinden opcode tarar. Reddedilen opcode aralıkları:

- `0x2a-0x2b`: `f32/f64.load`
- `0x38-0x39`: `f32/f64.store`
- `0x43-0x44`: `f32/f64.const`
- `0x5b-0x66`: f32/f64 karşılaştırma operatörleri
- `0x8b-0xa6`: f32/f64 aritmetik
- `0xa8-0xab`: `i32.trunc_f32/f64_s/u` (U-17'de eklendi)
- `0xae-0xb1`: `i64.trunc_f32/f64_s/u` (U-17'de eklendi)
- `0xb2-0xbf`: float dönüşümleri
- `0xFC` prefix sub-opcode `0x00-0x07`: saturating truncation (U-17'de eklendi)

Eşleşirse `Err(FloatOpcodes)` → modül reddedilir.

`skip_instruction()` LEB128 immediate'leri (max 5 byte u32) ve sabit boyutlu operandları (f32.const → 5B, f64.const → 9B) atlar; `0x0E br_table` LEB128 count + (count+1) etiket okur — bounds check ile buffer overread imkansız (Kani proof'larla doğrulanmış).

**Sınırlamalar:**
- Dead code'daki float opcode'lar da reddedilir (false positive — kabul edilen trade-off, soundness > completeness)
- Multi-byte instruction boundary heuristic — tam WASM grammar parser DEĞİL
- Encoded constant'lar içindeki float byte pattern'i tetikleyebilir (ileride wasmparser ile değiştirilecek, v1.5 hedefi)
...
4 sabit servis: COPY (bellek kopyalama, ~80c — Sprint U-14: stub, NotImplemented döner), CRC (CRC32 bit-by-bit, ~1500c — Sprint U-15 tahmini: 64B × 8 bit × ~3c), MAC (BLAKE3 keyed hash, ~350c), MATH (Q32.32 vektör dot product, ~200c). WCET hedefleri estimated, FPGA ölçümü pending.
...
`_start` (boot.S) → hart 0 seçimi → BSS sıfırlama → stack kurulumu → `rust_main`. `rust_main`, `boot::init()` çağırır (mtvec, PMP, blackbox, capability key, secure boot, task'lar, IPC ownership/seal), `self-test` açıksa test kodunu çalıştırır, sonra `boot::start()` timer'ı açıp scheduler'a girer. Multi-hart: hart 0 dışındakiler `wfi` park.

---

## 15. Formal Doğrulama — 200 Kani Harness + 7/7 TLA+

### 15.1 Proof Dağılımı

| Modül | Proof | Kapsam |
|-------|-------|--------|
| verify.rs (global) | 68 | DAL, PMP, bellek, cross-module invariantlar |
| sandbox (mod+allocator) | 24+1 | LEB128, float tarama (U-14/U-17: load/store, comparisons, br_table, 0xFC), bounds safety, allocator overlap |
...
| ipc (mod+blackbox) | 17+14 | CRC roundtrip, kanal ownership/sınırları, ring buffer wrap, blackbox record/CRC/wrap |
...
| **Toplam** | **200** | ~100 symbolic proof (kani::any ile state space tarar) + ~100 concrete/compile-time assertion |
...
Sabit kontroller Kani'den çıkarılıp `const _: () = assert!(...)` ile derleme zamanına taşındı — 0 runtime maliyet, koşul sağlanmazsa kod derlenmez: Token == 32B, IpcMessage == 64B, IPC_CHANNEL_SLOTS > 0, BlackboxRecord == BLACKBOX_RECORD_SIZE, BLACKBOX_MAX_RECORDS <= 255, SYSCALL_COUNT == 5, OTP_KEY_SIZE == 32, SIGNATURE_SIZE == 2 × OTP_KEY_SIZE (toplam 8 const assert).
...
Test suite'den önce çalışır. Bir tanesi bile fail ederse scheduler başlamaz — `wfi` loop ile halt. DO-178C DAL-A **design principles** PBIT (Power-on Built-In Test) önerir.

- **CRC32 engine**: Bilinen vektör "123456789" → `0xCBF43926` (IEEE 802.3). Eşleşmezse CRC motoru bozuk — tüm bütünlük kontrolü güvenilmez. HALT.
- **PMP integrity**: `read_pmpcfg0()` ile gerçek register okunup boot'ta kaydedilen shadow ile karşılaştırılır. Uyuşmazlık = register bozulması. HALT.
- **Policy engine sanity**: `decide_action(PmpFail, 0, 0)` → Shutdown dönmeli. Dönmezse policy engine bozuk — yanlış güvenlik kararı riski. HALT.
- **mstatus M-mode CSR access**: mstatus okunabiliyor mu, MPP bitleri reserved değil mi. HALT on fail.
- **mtvec set**: Trap handler kuruldu mu (`mtvec != 0`). HALT on fail.
- **BLAKE3 self-test**: Determinism (aynı input → aynı output) + non-zero output kontrolü. HALT on fail.
- **Ed25519 RFC 8032 TV1** (test-keys feature ile): Bilinen test vector ile imza doğrulama. HALT on fail.
- **CLINT timer ilerleme** (Sprint U-15): mtime register artıyor mu (donanım timer canlı mı). WARN level (QEMU TCG uyumluluğu).
- **misa ISA identity** (Sprint U-15): MXL=2 (RV64) + bit I/M/A/C set. WARN level.
...
Hardening kalemleri farklı tetiklenme noktalarında (per tick / per syscall / per ecall) çalıştığından "tek bir tick maliyeti" olarak özetlemek yanıltıcı olur. Bir scheduler tick'inde aktif olanların toplamı (PMP shadow ~5c + policy lockstep ~5c + watchdog ~1c + drift-free timer 0c ≈ ~11c) `WCET_SCHEDULER_TICK = 350c` bütçesi içinde kalır. Per-syscall maliyetler (kernel pointer sanitize, MPP doğrulama, syscall sayacı) ilgili syscall WCET hedeflerine dahil edilmiştir.
...
- **Verify:** `cargo kani` (200 harness), const assert (8+ derleme zamanı kontrol), TLC v2.19 (7 TLA+ spec, 35,770 state)
- **Supply chain:** `cargo audit` (RustSec CVE scan, 0 CVE) + `cargo deny check` (license/bans/sources policy)
- **CI:** GitHub Actions 5 job — clippy+build, QEMU self-test, supply chain audit, Kani full (master push), Kani critical subset (PR)
...
## 26. Performans Hedefleri (100MHz CVA6 — estimated, FPGA pending)

Sprint U-15'te recalibrated. Tüm değerler `src/common/config.rs` içindeki sabitlerle senkron — değişikliklerde tek kaynak orada.

| İşlem | Hedef | Karşılık @100MHz |
|-------|-------|------------------|
| trap_entry | ≤80 cycle | ≤0.80μs |
| trap_handler (Rust dispatch) | ≤80 cycle | ≤0.80μs |
| context_switch | ≤80 cycle | ≤0.80μs |
| scheduler_tick | ≤350 cycle | ≤3.50μs |
| sys_yield | ≤10 cycle | ≤0.10μs |
| ipc_recv | ≤40 cycle | ≤0.40μs |
| ipc_send | ≤60 cycle | ≤0.60μs |
| cap_invoke (cache hit yolu) | ≤25 cycle | ≤0.25μs |
| token_cache_hit | ≤10 cycle | ≤0.10μs |
| token_validate (BLAKE3 full) | ≤400 cycle | ≤4.00μs |
| compute_mac (BLAKE3) | ≤350 cycle | ≤3.50μs |
| compute_crc (CRC32 bit-by-bit) | ≤1500 cycle | ≤15.00μs |

Kesin ölçüm FPGA'da yapılacak. QEMU TCG'de rdcycle instruction count döner, gerçek cycle değil — WCET regresyonları sadece göreli karşılaştırma için kullanılır.
...
## Ek-6. IPC Head/Tail wrapping_add Güvenliği

`head + 1` / `tail + 1` yerine `head.wrapping_add(1)` ve `tail.wrapping_add(1)` — `overflow-checks = true` ile u16::MAX'ta panic engellenir. Modulo hâlâ doğru çalışır (Kani proof: `ipc_ring_buffer_wrap_never_exceeds_slots`).
...
*Sipahi Microkernel v1.0 (post-U-20) — 200 Kani Harness · 7/7 TLA+ Verified · 18 Hardening Sprint (U-3..U-20) · 0 Clippy Warning · 0 Runtime Panic · 0 Heap Allocation (kernel) · 0 Production Nested Fault*
