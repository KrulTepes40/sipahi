...
**Toplam:** ~7,200 satır · 42 kaynak dosya · 173 Kani proof  
...
| 4-5 | .data+bss+stack | RW + Lock | Yazılabilir veri + kernel stack |
| 6-7 | UART MMIO | RW + Lock | Serial port erişimi |
| 8-15 | Rezerve | — | Task stack PMP (per-task, context switch'te değişir) |
...
KernelBoot, PolicyRestart, PolicyDegrade, PolicyIsolate, PolicyFailover, PolicyShutdown, PmpFault, WatchdogTimeout, BlackboxFull. Post-mortem analiz için altın madeni.
...
Timer interrupt (code=7) → tick artır, scheduler çağır. ecall → syscall dispatch. Illegal instruction → log. U-mode ecall sonrası MPP doğrulama.
...
| sandbox | 20 | LEB128, float tarama, bounds safety, allocator |
...
| ipc | 15 | CRC roundtrip, kanal sınırları, SPSC no-overwrite, ring buffer wrap |
...
Yazılım seviyesinde çözülebilen 3/6 duvar tamamlandı. Kalan 2 donanım, 1 üretim seviyesi.
