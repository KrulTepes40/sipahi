...
**Toplam:** ~8,315 Rust + ~265 ASM satır · 42 kaynak dosya · 191 Kani harness · 7/7 TLA+ verified  
...
| Modül | Proof | Kapsam |
|-------|-------|--------|
| verify.rs (global) | 67 | DAL, PMP, bellek, cross-module invariantlar |
| sandbox (mod+allocator) | 20+1 | LEB128, float tarama (U-14: load/store + comparisons), bounds safety, allocator overlap |
...
| ipc (mod+blackbox) | 15+14 | CRC roundtrip, kanal sınırları, ring buffer wrap, blackbox record/CRC/wrap |
...
| **Toplam** | **191** | 90 symbolic proof (kani::any ile state space tarar) + 101 concrete/compile-time assertion |
...
- **Verify:** `cargo kani` (191 harness), const assert (8 derleme zamanı kontrol), TLC (7 TLA+ spec)
